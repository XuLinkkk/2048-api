...
epsilon = 0.9  
mem_capacity = 6000  
depth1 = 128  #convolution layer1 depth
depth2 = 128  #convolution layer2 depth
batch_size = 512  
input_units = 16  
hidden_units = 256  #teh fully connected layer neurons
output_units = 4  #output neurons:directions

#store states and lables of the game
replay_memory = list()  #states of the game
replay_labels = list()  #labels of the states
...
single_output = model(single_dataset)  #for single example

logits = model(tf_batch_dataset)  #batch data
...
J = []  #loss
scores = []  #scores
final_parameters = {}  #store final parameters
M = 10000  #number of episodes

controls = {0: up, 1: left, 2: right, 3: down}  #four directions
...
with tf.Session() as session:
    ...
    for ep in range(M):
        ...
        while finish == 'not over':
            ...
            # change the map values and store them in memory
            prev_state = deepcopy(prev_board)
            prev_state = change_the_map(prev_state)
            ...
            if len(replay_memory) >= mem_capacity:
                ...
                # store the final weights in the dictionary
        ...
        print("Episode {} finished with score {}, result : {} board : {}, epsilon  : {}, learning rate : {} ".format(ep,
                                                                                                                     total_score,
                                                                                                                     finish,
                                                                                                                     board,
                                                                                                                     epsilon,
